...
# course key -> (display, course_map, standards_file, ctso_area)
...
COURSES = {
    "acs_i": ("ACS I", "acs1_course_map_grounded.json", "nv_cs_standards_grounded.json", "1.0"),
    "wdd_i": ("WDD I", "wdd_course_map_grounded.json", "wdd_cs_standards_grounded.json", "1.0"),
    "dgd_i": ("DGD I", "dgd_course_map_grounded.json", "dgd_cs_standards_grounded.json", "1.0"),
    "dgd_ii": ("DGD II", "dgd2_course_map_grounded.json", "dgd_cs_standards_grounded.json", "1.0"),
    "cet": ("CET", "cet_course_map_grounded.json", "cet_cs_standards_grounded.json", None),
}
...
def main() -> None:
    ...
    for key, (display, cmap, sfile, ctso) in COURSES.items():
        ...
        core_covered = covered & core_codes
        core_gaps = sorted(core_codes - covered)
        ctso_covered = covered & ctso_codes

        rows.append(
            f"| {display} | {len(core_covered)}/{len(core_codes)} | "
            f"{pct(len(core_covered), len(core_codes))} | "
            f"{len(core_gaps)} | "
            f"{(str(len(ctso_covered)) + '/' + str(len(ctso_codes))) if ctso_codes else '—'} |"
        )

        # Per-course gap detail.
        lines = [f"### {display} — {len(core_gaps)} uncovered core indicator(s)", ""]
        if core_gaps:
    ...
    out = [
        ...
        "grounded crosswalk JSON. **Core** excludes the CTSO/FBLA area (1.0), "
        "reported separately as co-curricular. Mechanical coverage only — see "
        "`acs1_standards_gap_status.md` / `wdd_standards_gap_status.md` for "
        "L1/L2 scope judgment on which gaps are genuine.",
        "",
        ...
        "## Summary",
        "",
        "| Course | Core covered | Core % | Core gaps | CTSO (1.0) tagged |",
        "|---|---|---|---|---|",
        *rows,
        "",
